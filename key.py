...
# Check if old solutions API is present, otherwise initialize modern MediaPipe vision tasks
try:
    ...
    # Automatically download the required hand landmarker model file if missing
    model_path = 'hand_landmarker.task'
    if not os.path.exists(model_path):
        print("Downloading required MediaPipe model file...")
...
while True:
    ...
    if USE_LEGACY:
        ...
        if results.multi_hand_landmarks and results.multi_handedness:
            for handLms, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                ...
                hand_data.append((label, handLms.landmark))
    else:
    ...
    for label, landmarks in hand_data:
        thumb = landmarks[4]
        index = landmarks[8]
